...
exports.postSearch = function(req, res, next) {
  extractSearchQueryFromReq(req.body).then(function (query) {
    //Group 3 Added Modified
    //Added the decimallatitude decimallongitude geodeticdatum to query list for its DB retrieval
    ...
    MetaDataInformationModel.find(query, columns, async function(err, metaDatas){
      if(err){
        ...
        if(typeof req.body.location !== 'undefined' && req.body.location !== null && req.body.location !== ''){
          // Using the points within the polygon algorithm to find coordinates within the polygon.Passing the drawn polygon coordinates
          // and the coordinates from the DB to find it's points within.
           ...
           for(var metaData of metaDatas){
            ...
            //Calling Turf.js points in polygon algorithm
